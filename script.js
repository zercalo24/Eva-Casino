...
/* ═══════════════════════════════════════════════════════════════
   МОДУЛЬ 3: СРАВНЕНИЕ КАЗИНО
   ═══════════════════════════════════════════════════════════════
   Нужная HTML-структура в строке таблицы:

   <tr class="ct-row" data-casino-id="vavada" data-casino-name="Vavada Casino">
     ...
     <td>
       <button class="btn-compare" aria-label="Сравнить Vavada Casino">+ Сравнить</button>
     </td>
   </tr>

   — или в карточке:
   <div class="casino-card" data-casino-id="duel" data-casino-name="Duel Casino">
     <button class="btn-compare">+ Сравнить</button>
   </div>

   ─────────────────────────────────────────────────────────────
   Атрибуты:
   • data-casino-id   — уникальный slug казино (vavada, duel, ...)
   • data-casino-name — читаемое название для панели
   ═══════════════════════════════════════════════════════════════ */

function initCompare() {
  const STORAGE_KEY = 'ce_compare';
  const MAX_COMPARE = 4;

  // ── Состояние ──────────────────────────────────────────────
  let selected = loadFromStorage();

  // ── Создаём плавающую панель ───────────────────────────────
  const panel = createPanel();
  document.body.appendChild(panel);

  injectStyle('compare-style', `
    /* Кнопки сравнения */
    .btn-compare {
      padding: 5px 12px;
    ...
    .btn-compare:hover { border-color: #e02020; color: #e02020; }
    .btn-compare.added {
      background: #e8f9ef;
      border-color: #27ae60;
      color: #27ae60;
    ...
    .cp-left { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
    .cp-title {
      font-size: 14px;
      font-weight: 700;
      white-space: nowrap;
    }
    .cp-title span { color: #f5a623; }
    ...
    .cp-tag-remove:hover { color: #e02020; }

    .cp-right { display: flex; align-items: center; gap: 10px; }
    .cp-btn-compare {
      padding: 10px 22px;
      background: #e02020;
      color: #fff;
    ...
    .cp-btn-compare:hover { background: #c01818; }
    .cp-btn-clear {
      padding: 10px 14px;
      background: rgba(255,255,255,.08);
      border: 1px solid rgba(255,255,255,.15);
    ...
    .cp-btn-clear:hover { background: rgba(255,255,255,.15); color: #fff; }
  ...
  function showWarning(msg) {
    ...
    warningTimer = setTimeout(() => warning.classList.remove('visible'), 2500);
  }
  ...
  function createPanel() {
    ...
    p.setAttribute('aria-label', 'Панель сравнения казино');
    p.innerHTML = `
      <div class="cp-left">
        <div class="cp-title">Сравнить выбранные: <span id="cp-count">0</span></div>
        <div class="cp-tags" id="cp-tags"></div>
      </div>
      <div class="cp-right">
        <button class="cp-btn-clear" id="cp-clear">Очистить</button>
        <button class="cp-btn-compare" id="cp-go">⚖️ Сравнить</button>
      </div>
    `;
    return p;
  }
  ...
  function render() {
    // Счётчик
    panel.querySelector('#cp-count').textContent = selected.length;

    // Тэги с казино
    const tagsEl = panel.querySelector('#cp-tags');
    tagsEl.innerHTML = '';
    selected.forEach(item => {
      ...
      tag.innerHTML = `
        ...
        <button class="cp-tag-remove" data-id="${escapeHtml(item.id)}" aria-label="Убрать ${escapeHtml(item.name)}">×</button>
      `;
    ...
    // Обновляем кнопки .btn-compare на странице
    document.querySelectorAll('.btn-compare').forEach(btn => {
      const row = btn.closest('[data-casino-id]');
      if (!row) return;
      const id = row.dataset.casinoId;
      const isAdded = selected.some(s => s.id === id);
      btn.textContent = isAdded ? '✓ Добавлено' : '+ Сравнить';
      btn.classList.toggle('added', isAdded);
      btn.setAttribute('aria-pressed', isAdded ? 'true' : 'false');
    });
  }

  // ── Добавить / убрать казино ───────────────────────────────
  function toggleCasino(id, name) {
    const idx = selected.findIndex(s => s.id === id);
    if (idx > -1) {
      selected.splice(idx, 1);
    } else {
      if (selected.length >= MAX_COMPARE) {
        showWarning(`Максимум ${MAX_COMPARE} казино для сравнения`);
        return;
      }
  ...
  // ── Обработчики кликов ─────────────────────────────────────
  // Кнопки «+ Сравнить» в таблице / карточках
  document.addEventListener('click', (e) => {
    ...
    if (!row) return;
    toggleCasino(
      row.dataset.casinoId,
      row.dataset.casinoName || row.dataset.casinoId
    );
  });

  // Кнопка «×» удаления тэга
  document.addEventListener('click', (e) => {
    const removeBtn = e.target.closest('.cp-tag-remove');
    if (!removeBtn) return;
  ...
  document.getElementById('cp-go').addEventListener('click', () => {
    if (selected.length < 2) {
      showWarning('Выберите минимум 2 казино для сравнения');
      return;
    }
    const ids = selected.map(s => s.id).join(',');
    window.location.href = `/compare?casino=${ids}`;
  });

  // ── Первый рендер (восстановление из localStorage) ─────────
  render();
}
